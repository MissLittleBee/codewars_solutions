...
completed at: 2019-03-05 09:49:07
by: Jakub Červinka
...
def catalog(s, article):
    ...
    prods = [[prod.find('name').text,(prod.find('prx').text, prod.find('qty').text)] for prod in soup.find_all("prod")]
    result_list = ['{} > prx: ${} qty: {}'.format(key,*val) for key, val in prods if article in key] 
    return '\r\n'.join(result_list) if len(result_list) > 0 else 'Nothing'
