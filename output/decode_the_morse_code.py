...
completed at: 2019-02-03 16:40:22
by: Jakub Červinka
...
def decodeMorse(message):
    message = message.strip().replace(' ','x').replace('xxx','x x').split('x')
    return ''.join([MORSE_CODE[code] if code is not ' ' else ' ' for code in message])
