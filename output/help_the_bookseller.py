...
completed at: 2019-04-25 08:07:27
by: Jakub Červinka
...
def stock_list(listOfArt, listOfCat):
    books = {item.split(' ')[0] : int(item.split(' ')[1]) for item in listOfArt}
    result = []
    amounts = []
    for cat in listOfCat:
        amount = sum((books[key] for key in books if key[0] == cat))
        amounts.append(amount)
        result.append(f'({cat} : {amount})')
    return ' - '.join(result) if sum(amounts) > 0 else ''
