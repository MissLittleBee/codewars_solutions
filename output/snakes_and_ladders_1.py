"""Kata - Snakes and Ladders

completed at: 2023-06-16 11:49:24
by: Jakub Červinka
...
class Player:
    ...
    def __str__(self):
        return f'Player {self.num} is on square {self.pos}'
...
class SnakesLadders():
    ...
    def __init__(self):
        self.players = itertools.cycle((Player(1), Player(2)))
    ...
    def play(self, die1, die2):
        if self.winner:
            return 'Game over!'
        amt = die1 + die2
        self.current_player.pos += amt
        if self.current_player.pos == 100:
            self.winner = True
            return f'Player {self.current_player.num} Wins!'
        elif self.current_player.pos > 100:
            diff = self.current_player.pos - 100
            self.current_player.pos -= diff * 2
        # shortcut
        self.current_player.pos = self.shortcuts.get(
