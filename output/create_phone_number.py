...
completed at: 2019-03-06 12:10:04
by: Jakub Červinka
...
def create_phone_number(list_of_numbers):
    first = ''.join(str(c) for c in list_of_numbers[:3])
    second = ''.join(str(c) for c in list_of_numbers[3:6])
    last = ''.join(str(c) for c in list_of_numbers[6:])
