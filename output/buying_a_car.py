...
completed at: 2022-06-21 20:37:14
by: Jakub Červinka
...
def nbMonths(old_car_value, new_car_value, saving_per_month, percent_loss_by_month):
    ...
    savings = 0
    perc_lost = percent_loss_by_month / 100
    
    for month_num in count(1):
        
        if month_num % 2 == 0:
            perc_lost += 0.005
            
        old_car_value *= (1 - perc_lost)
        new_car_value *= (1 - perc_lost)
        savings += saving_per_month
        
        total_cash = savings + old_car_value
        
        # print(f'end month: {month_num}: percent_loss {perc_lost} available {total_cash - new_car_value}')
        
        if total_cash >= new_car_value:
            return [month_num, int(round(total_cash - new_car_value))]
