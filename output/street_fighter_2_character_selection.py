"""Kata - Street Fighter 2 - Character Selection

completed at: 2022-09-19 19:36:59
by: Jakub Červinka
...
def street_fighter_selection(fighters, initial_position, moves):
    ...
    print(initial_position, moves)
    
    if not moves:
        return []    
    
    top, bottom = deque(fighters[0]), deque(fighters[1])
    selection, rotation = initial_position
    top.rotate(-rotation)
    bottom.rotate(-rotation)
    current = bottom if selection else top
